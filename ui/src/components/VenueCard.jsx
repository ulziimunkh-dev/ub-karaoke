...
const VenueCard = ({ venue, onBook, distance }) => {
    ...
    // Get all display images (valid images + fallback to featured/default)
    const displayImages = validImages.length > 0 ? validImages : [featuredImage];
    ...
    return (
        <div className={`group bg-[#151521] rounded-2xl overflow-hidden border border-white/5 transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_0_30px_rgba(0,0,0,0.5)] relative flex flex-col h-full ${!isOpen ? 'opacity-80 grayscale-[0.8]' : ''}`}>
            ...
            >
                ...
                {displayImages.length > 1 && (
                    <>
                        <button
                            ...
                            className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-black/50 hover:bg-black/70 rounded-full flex items-center justify-center text-white transition-all z-20 opacity-0 group-hover:opacity-100"
                        >
                            <i className="pi pi-chevron-left text-sm"></i>
                        </button>
                        <button
                            onClick={handleNextImage}
                            className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-black/50 hover:bg-black/70 rounded-full flex items-center justify-center text-white transition-all z-20 opacity-0 group-hover:opacity-100"
                        >
                ...
                {/* Photo Count Badge */}
                {displayImages.length > 1 && (
                    <div className="absolute bottom-2 right-2 bg-black/60 backdrop-blur-sm px-2 py-1 rounded-lg flex items-center gap-1.5 text-white text-[10px] font-bold border border-white/10 z-20">
                        <i className="pi pi-images text-[10px]"></i>
