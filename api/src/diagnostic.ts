...
async function bootstrap() {
    ...
    for (const manager of managers) {
        ...
        const allVenues = await venuesService.findAll({ organizationId: manager.organizationId, includeInactive: true });
        console.log(`Venues for Org ${manager.organizationId}: ${allVenues.length}`);
        allVenues.forEach(v => {
            console.log(` - [${v.isActive ? 'ACTIVE' : 'INACTIVE'}] ID: ${v.id}, Name: ${v.name}, Rooms: ${v.rooms?.length || 0}`);
        });

        // 3. Fetch venues with ONLY active filter (to see if it hides some)
        const activeVenues = await venuesService.findAll({ organizationId: manager.organizationId, includeInactive: false });
