...
import { VenueOperatingHours, DayOfWeek } from '../modules/venues/entities/venue-operating-hours.entity';
...
export const seed = async (dataSource: DataSource) => {
    ...
    const orgs = [
        { code: 'UBK-EXP', name: 'Expired Org', plan: plans['GROWTH'], status: 'active', startedAt: new Date(Date.now() - 31 * 24 * 60 * 60 * 1000), endsAt: new Date(Date.now() - 24 * 60 * 60 * 1000) },
        { code: 'UBK-NOP', name: 'No Plan Org', plan: undefined, status: 'active' },
        { code: 'UBK-ACT', name: 'Active Org', plan: plans['GROWTH'], status: 'active', startedAt: new Date() }
    ];
    ...
    const staffData = [
        { email: 'manager1@ubkaraoke.mn', username: 'manager1', password: managerPass, name: 'UBK Manager', role: StaffRole.MANAGER, org: savedOrgs['UBK-EXP'] },
        { email: 'staff1@ubkaraoke.mn', username: 'staff1', password: staffPass, name: 'UBK Front Desk', role: StaffRole.STAFF, org: savedOrgs['UBK-EXP'] },
        { email: 'manager2@ubkaraoke.mn', username: 'manager2', password: managerPass, name: 'Star Manager', role: StaffRole.MANAGER, org: savedOrgs['UBK-NOP'] },
        { email: 'staff2@ubkaraoke.mn', username: 'staff2', password: staffPass, name: 'Star Assistant', role: StaffRole.STAFF, org: savedOrgs['UBK-NOP'] },
        { email: 'manager3@ubkaraoke.mn', username: 'manager3', password: managerPass, name: 'Active Manager', role: StaffRole.MANAGER, org: savedOrgs['UBK-ACT'] },
        { email: 'staff3@ubkaraoke.mn', username: 'staff3', password: staffPass, name: 'Active Staff', role: StaffRole.STAFF, org: savedOrgs['UBK-ACT'] },
    ];
    ...
    const customers = [
        { email: 'bat@gmail.com', username: 'customer', password: custPass, name: 'Bat-Erdene', phone: '99998888', role: UserRole.CUSTOMER },
        { email: 'saraa@gmail.com', username: 'saraa', password: custPass, name: 'Saranduya', phone: '88887777', role: UserRole.CUSTOMER },
    ];
    ...
    for (const name of roomTypes) {
        const rt = roomTypeRepo.create({ name, createdBy: adminId, updatedBy: adminId });
    ...
    for (const f of features) {
        const rf = roomFeatureRepo.create({ ...f, createdBy: adminId, updatedBy: adminId });
