...
import {
  VenueOperatingHours,
  DayOfWeek,
} from '../venues/entities/venue-operating-hours.entity';
...
export class BookingsService {
  ...
  async create(
    ...
    return await this.bookingsRepository.manager.transaction(
      async (transactionalEntityManager) => {
        ...
        if (createBookingDto.pointsToUse && createBookingDto.pointsToUse > 0) {
          if (!userId) {
            throw new BadRequestException(
              'Must be logged in to use loyalty points',
            );
          }
          ...
          const maxAllowedDiscount = createBookingDto.totalPrice * 0.5; // Max 50%

          if (discountValue > maxAllowedDiscount) {
            throw new BadRequestException(
              `Loyalty points can only cover up to 50% of the total price (Max: ${maxAllowedDiscount} MNT)`,
            );
          }
        ...
        if (createBookingDto.promoCode) {
          ...
          const firstRoom = await transactionalEntityManager
            .getRepository(Room)
            .findOne({
              where: { id: roomIds[0] },
              relations: ['venue'],
          ...
          if (firstRoom?.venue) {
            try {
              ...
              const priceAfterLoyalty =
                createBookingDto.totalPrice - loyaltyDiscount;
              if (promotion.discountType === 'PERCENT') {
                promoDiscount = Math.round(
                  (priceAfterLoyalty * Number(promotion.value)) / 100,
                );
              } else {
              ...
              throw new BadRequestException(
                error.message || 'Invalid promotion code',
              );
            }
          }
        }
        ...
        for (const roomId of roomIds) {
          ...
          const startTime = new Date(
            `${startDate}T${createBookingDto.startTime}`,
          );
          const endTime = new Date(`${startDate}T${createBookingDto.endTime}`);
          ...
          if (durationHours < Number(venue.minBookingHours || 1)) {
            throw new BadRequestException(
              `Minimum booking duration is ${venue.minBookingHours || 1} hours`,
            );
          }
          if (durationHours > Number(venue.maxBookingHours || 6)) {
            throw new BadRequestException(
              `Maximum booking duration is ${venue.maxBookingHours || 6} hours`,
            );
          }
          ...
          const booking = transactionalEntityManager
            ...
            .create({
              ...
              source:
                (createBookingDto.source as BookingSource) ||
                BookingSource.ONLINE,
              organizationId: venue.organizationId,
              appliedPromotionId: appliedPromoId,
            });

          const savedBooking = await transactionalEntityManager
            .getRepository(Booking)
            .save(booking);
          bookings.push(savedBooking);

          // Status history inside transaction
          const history = transactionalEntityManager
  ...
  private async checkTimeConflicts(
    ...
    const roomRepo = manager
      ? manager.getRepository(Room)
    ...
    const query = repo
      ...
      .andWhere(
        new Brackets((qb) => {
          qb.where('booking.status != :reserved', {
  ...
  async getAvailableSlots(roomId: string, date: string): Promise<any[]> {
    ...
    return slots
      ...
      .map((time) => {
        ...
        const closing = (endH <= startH ? endH + 24 : endH) + endM / 60;
        const venueClosingTime = new Date(
          new Date(`${date}T00:00:00`).getTime() + closing * 3600000,
        );
        let maxHours =
          (venueClosingTime.getTime() - slotStart.getTime()) / 3600000;

        // 2. Cap by next booking/manual block
        const allConflicts = [
          ...existingBookings.map(
            (b) =>
              new Date(new Date(b.startTime).getTime() - bufferMinutes * 60000),
          ),
          ...blockedSlots.map((b) => new Date(`${date}T${b.startTime}`)),
        ]
          .filter((d) => d > slotStart)
          .sort((a, b) => a.getTime() - b.getTime());

        if (allConflicts.length > 0) {
          const nextConflict = allConflicts[0];
          const hoursToNext =
            (nextConflict.getTime() - slotStart.getTime()) / 3600000;
  ...
  async sendExpirationReminders(): Promise<number> {
    ...
    const expiringBooks = await this.bookingsRepository
      ...
      .andWhere('booking.expiresAt BETWEEN :now AND :oneMin', {
        now,
        oneMin: oneMinuteLater,
      })
    ...
    for (const booking of expiringBooks) {
      const minutesRemaining = Math.floor(
        (booking.expiresAt.getTime() - now.getTime()) / (60 * 1000),
      );
