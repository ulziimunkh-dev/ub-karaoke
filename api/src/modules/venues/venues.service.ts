...
export class VenuesService {
  ...
  async findAll(filters?: {
    ...
    query.addOrderBy('room.sortOrder', 'ASC').addOrderBy('room.name', 'ASC');
  ...
  private async syncOperatingHours(
    venueId: string,
    ...
    for (const [key, range] of Object.entries(openingHours)) {
      if (key !== 'Daily' && daysMap[key]) {
        ...
        if (open && close) {
          ...
          const existingIdx = hoursToInsert.findIndex(
            (h) => h.dayOfWeek === daysMap[key],
          );
          if (existingIdx >= 0) {
