...
export class RoomsService {
  ...
  async create(
    ...
    if (roomFeatureIds?.length) {
      room.roomFeatures =
        await this.featuresRepository.findByIds(roomFeatureIds);
    }
  ...
  async addPricing(
    roomId: string,
    pricingData: Partial<RoomPricing>,
    user: any,
  ) {
  ...
  async removePricing(id: string, user: any) {
    ...
    if (pricing) {
      ...
      if (
        user &&
        user.role !== 'sysadmin' &&
        pricing.organizationId !== user.organizationId
      ) {
  ...
  async removeImage(id: string, user: any) {
    ...
    if (image) {
      if (
        user &&
        user.role !== 'sysadmin' &&
        image.organizationId !== user.organizationId
      ) {
  ...
  async updateSortOrders(
    orders: { roomId: string; sortOrder: number }[],
    user: any,
  ) {
