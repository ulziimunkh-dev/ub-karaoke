...
export class QpayService {
  ...
  constructor(
    ...
    this.callbackUrl = this.configService.get<string>(
      'QPAY_CALLBACK_URL',
      'http://localhost:3001/payments/qpay/callback',
    );
  }
  ...
  private async doAuthenticate(): Promise<string> {
    this.logger.log(
      `Authenticating with QPay... (clientId: ${this.clientId}, baseUrl: ${this.baseUrl})`,
    );
    ...
    try {
      ...
      const responseData =
        error.response?.data || error.response || 'No response body';
      this.logger.error(
        `QPay authentication failed - clientId: ${this.clientId}, url: ${this.baseUrl}/v2/auth/token`,
      );
      this.logger.error(
        `QPay auth error response: ${JSON.stringify(responseData)}`,
      );
      throw error;
    }
  }
  ...
  async createInvoice(
    ...
    this.logger.log(
      `Creating QPay invoice: ${senderInvoiceNo}, amount: ${amount}`,
    );
  ...
  async getInvoice(invoiceId: string): Promise<any> {
    ...
    const { data } = await firstValueFrom(
      this.httpService.get(`${this.baseUrl}/v2/invoice/${invoiceId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      }),
  ...
  async cancelInvoice(invoiceId: string): Promise<any> {
    ...
    const { data } = await firstValueFrom(
      this.httpService.delete(`${this.baseUrl}/v2/invoice/${invoiceId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      }),
