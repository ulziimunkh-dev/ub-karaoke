...
export class FinanceController {
    ...
    async getTotalEarnings(@Req() req: any, @Query('status') status?: string, @Query('organizationId') filtersOrgId?: string) {
        if (!req.user.organizationId && req.user.role !== 'sysadmin') {
            throw new ForbiddenException('Not associated with an organization');
        }
    ...
    async requestPayout(@Req() req: any, @Body() data: { earningIds: string[], payoutAccountId: string }) {
        ...
        return this.organizationsService.createPayout(req.user.organizationId, {
            payoutAccountId: data.payoutAccountId,
            status: 'PENDING' as any
        }, data.earningIds, req.user.id);
    }

    @Get('payout-accounts')
    @ApiOperation({ summary: 'Get payout accounts' })
    async getPayoutAccounts(@Req() req: any, @Query('organizationId') filtersOrgId?: string) {
        if (!req.user.organizationId && req.user.role !== 'sysadmin') {
            throw new ForbiddenException('Not associated with an organization');
        }
        const orgId = req.user.role === 'sysadmin' && filtersOrgId
    ...
    async addPayoutAccount(@Req() req: any, @Body() data: any) {
        if (!req.user.organizationId) {
            throw new ForbiddenException('Only organization members can add payout accounts');
        }
        return this.organizationsService.addPayoutAccount(req.user.organizationId, data, req.user.id);
    }

    @Patch('payout-accounts/:id')
    @ApiOperation({ summary: 'Update payout account' })
    async updatePayoutAccount(@Req() req: any, @Param('id') id: string, @Body() data: any) {
    ...
    async updatePayoutStatus(@Req() req: any, @Param('id') id: string, @Body('status') status: string) {
