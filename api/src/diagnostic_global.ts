...
async function bootstrap() {
  ...
  orgs.forEach((o) =>
    console.log(`  - ${o.name} (isActive: ${o.isActive}, status: ${o.status})`),
  );
  ...
  venues.forEach((v) =>
    console.log(
      `  - ${v.name} (isActive: ${v.isActive}, orgActive: ${v.organization?.isActive})`,
    ),
  );
  ...
  rooms.forEach((r) =>
    console.log(
      `  - ${r.name} (isActive: ${r.isActive}) -> Venue: ${r.venue?.name}`,
    ),
  );
